!(function(root, factory) {
    ...
    Object.keys ||
        (Object.keys = ((hasOwnProperty = Object.prototype.hasOwnProperty),
    ...
    function mergeDialogOptions(className, labels, properties, args) {
        var locale, swapButtons, baseOptions;
        args &&
            args[0] &&
    ...
    function createLabels(labels, locale) {
        for (var buttons = {}, i = 0, j = labels.length; i < j; i++) {
            var argument = labels[i],
                key = argument.toLowerCase(),
                value = argument.toUpperCase();
            buttons[key] = { label: getText(value, locale) };
        }
        return buttons;
    }
    ...
    function sanitize(options) {
        ...
        return (
            (options = $.extend({}, defaults, options)).buttons ||
                (options.buttons = {}),
            ...
            each(buttons, function(key, button, index) {
                ...
                if ((button.label || (button.label = key), !button.className)) {
                    var isPrimary = !1;
                    (isPrimary = options.swapButtonOrder
                        ? 0 === index
                        : index === total - 1),
                        (button.className =
                            total <= 2 && isPrimary
    ...
    function processCallback(e, dialog, callback) {
        ...
        e.stopPropagation(),
            e.preventDefault(),
            ($.isFunction(callback) && !1 === callback.call(dialog, e)) ||
                dialog.modal('hide');
    }
    function validateMinOrMaxValue(
        input,
        type,
        name,
        value,
            ...
            if ('date' === type)
                input.attr(name, value),
                    /(\d{4})-(\d{2})-(\d{2})/.test(value) ||
                        console.warn(
                            'Browsers which natively support the "date" input type expect date values to be of the form "YYYY-MM-DD"  (see ISO-8601 https://www.iso.org/iso-8601-date-and-time-format.html). Bootbox does not enforce this rule, but your ' +
                                name +
    ...
    return (
        ...
        (exports.dialog = function(options) {
            ...
            if (
                ((options = sanitize(options)), $.fn.modal.Constructor.VERSION)
            ) {
            ...
            if (
                ...
                getKeyLength(options.buttons) > 0 &&
                    ...
                    body.after(footer)),
                !0 === options.animate && dialog.addClass('fade'),
                options.className && dialog.addClass(options.className),
                options.size &&
                    (options.fullBootstrapVersion.substring(0, 3) < '3.1' &&
                        console.warn(
            ...
            return (
                options.centerVertical &&
                    (options.fullBootstrapVersion < '4.0.0' &&
                        console.warn(
                ...
                dialog.one('hide.bs.modal', function(e) {
                    e.target === this &&
                        (dialog.off('escape.close.bb'), dialog.off('click'));
                }),
                ...
                'static' !== options.backdrop &&
                    dialog.on('click.dismiss.bs.modal', function(e) {
                        dialog.children('.modal-backdrop').length &&
                ...
                options.show && dialog.modal('show'),
                dialog
            );
        }),
        ...
        (exports.prompt = function() {
            var options, promptDialog, form, input, shouldShow, inputOptions;
            if (
                ((form = $(templates.form)),
                (options = mergeDialogOptions(
                    'prompt',
                ...
                options.inputType || (options.inputType = defaults.inputType),
                (shouldShow =
                    options.show === undefined ? defaults.show : options.show),
                (options.show = !1),
                (options.buttons.cancel.callback = options.onEscape = function() {
                ...
                (options.buttons.confirm.callback = function() {
                    ...
                    if ('checkbox' === options.inputType)
                        value = input
                            ...
                            .get();
                    else if ('radio' === options.inputType)
                        value = input.find('input:checked').val();
                    else {
                        if (input[0].checkValidity && !input[0].checkValidity())
                            return !1;
                        value =
                            'select' === options.inputType &&
                            !0 === options.multiple
                                ? input
            ...
            switch (
                ...
                case 'password':
                    input.val(options.value),
                        ...
                        options.required && input.prop({ required: !0 });
                    break;
                case 'date':
                case 'time':
                case 'number':
                case 'range':
                    if (
                        ...
                        options.pattern &&
                            input.attr('pattern', options.pattern),
                        options.required && input.prop({ required: !0 }),
                        'date' !== options.inputType && options.step)
                    ) {
                        if (
                            !(
                                'any' === options.step ||
                ...
                case 'select':
                    ...
                    options.placeholder &&
                        input.attr('placeholder', options.placeholder),
                        options.required && input.prop({ required: !0 }),
                        options.multiple && input.prop({ multiple: !0 }),
                ...
                case 'checkbox':
                    var checkboxValues = $.isArray(options.value)
                        ? options.value
                        : [options.value];
                    if (!(inputOptions = options.inputOptions || []).length)
                        throw new Error(
            ...
            if (
                ...
                form.prepend(message), (options.message = form);
            } else options.message = form;
            return (
                (promptDialog = exports.dialog(options)).off('shown.bs.modal'),
                promptDialog.on('shown.bs.modal', function() {
                    input.focus();
                }),
                !0 === shouldShow && promptDialog.modal('show'),
                promptDialog
            );
        }),
        exports.addLocale('en', { OK: 'OK', CANCEL: 'Cancel', CONFIRM: 'OK' }),
        exports
    );
});
